    ...
    <div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
        ...
        <div class="px-4 py-4 border-b border-gray-200 md:px-6 md:py-5">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-base font-semibold text-gray-800 md:text-lg">Edit Mahasiswa</h3>
        ...
        <div class="p-4 md:p-6">
            <form action="{{ route('mahasiswa.update', $mahasiswa) }}" method="POST" class="space-y-5 md:space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="relative mt-1.5">
                        <select name="jurusan_id" required
                            ...
                            @foreach ($jurusan as $major)
                                <option value="{{ $major->id }}" {{ $mahasiswa->jurusan_id == $major->id ? 'selected' : '' }}>{{ $major->nama_jurusan }}</option>
                            @endforeach
                        </select>
